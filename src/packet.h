...
// Return pointer to data on success or NULL on error.
...
// Return -1 on error, TRUE if is START packet, else FALSE.
int isStartPacket(unsigned char *packet);

// Check if packet is an END packet.
// Return -1 on error, TRUE if is END packet, else FALSE.
